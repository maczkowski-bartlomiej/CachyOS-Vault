...
| Environment | `custom-configs/Environment/90-gaming.conf` | `~/.config/environment.d/90-gaming.conf` | none | systemd user environment.d | Enables NVIDIA shader disk cache and sets a 12 GB shader cache size for gaming sessions; takes effect on new login/session. |
